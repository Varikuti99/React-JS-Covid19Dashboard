...
class Home extends Component {
  ...
  // STEP 2: here we going render by updating state

  componentDidMount() {
    this.getAllData()
  }

  // STEP 3: Hear we going to fetch data from url and we going check whether response is Success we going get data of states and cases
  getAllData = async () => {
    ...
    if (response.ok === true) {
      ...
      // Here we run for loop and take each state from states list and add con,dec,rec cases to national cases india
  ...
  // STEP 1: It going to filter the states based on input entered
  changeSearchInput = event => {
    ...
    // Hear we going to set search input and search results to state
  ...
  // STEP 4: Here we going display cases of all over india
  renderAllNationalData = () => {
    ...
    return (
      <>
        <div testid="countryWideConfirmedCases" className="stats-block">
          ...
          />
          <p className="stats-title red">{totalConfirmedCases}</p>
        </div>

        <div testid="countryWideActiveCases" className="stats-block">
          ...
          />
          <p className="stats-title blue">{totalActiveCases}</p>
        </div>

        <div testid="countryWideRecoveredCases" className="stats-block">
          ...
          />
          <p className="stats-title green">{totalRecoveredCases}</p>
        </div>

        <div testid="countryWideDeceasedCases" className="stats-block">
          ...
          />
          <p className="stats-title gray">{totalDeceasedCases}</p>
        </div>
      </>
    )
  }

  // STEP 6: Here we sort states in table  in asc or desc order
  ...
  // STEP 5: Here we created table header
  // and Here we going to pass props to total Stats all states will be displayed in table
  renderAllStatesList = () => {
    const {statesInfo} = this.state
    return (
  ...
  // STEP 7: Here we going pass props and show search suggestions container in search bar
  showSearchList = () => {
  ...
  render() {
    const {isLoading, filteredSearchList, searchInput} = this.state
...
export default Home
// 265 testid="countryWideConfirmedCases"
// 275 testid="countryWideActiveCases"
// 285 testid="countryWideRecoveredCases"
// 295 testid="countryWideDeceasedCases"
// 388 testid="homeRouteLoader"
// 338 testid="stateWiseCovidDataTable"
// 421 testid="searchIcon"
// 401 testid="searchResultsUnorderedList"
